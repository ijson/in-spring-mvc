 ...
 * @Copyright(c)2015:北京荣程创新科技发展有限公司
 */
public class Validator {
	 ...
	 * @update Jul 3, 2015
	 */
	public static boolean isEmpty(@SuppressWarnings("rawtypes")
	List list) { //
	...
	public static Properties getProperties(String filePath, String fileName) {
		...
		try {
			...
			try {
				...
				System.out.println("文件读取失败 key=" + e1.getMessage());
			}
		} finally {
			try {
				if (null != fileInput)
	...
	/**
	 * description:  发布模式使用(返回classes的绝对路径)
	 * @return  
	 * @author cuiyongxu 
	 * @update Nov 17, 2015
	 */
	public static String getClassLoaderPath0() {
		String path = Validator.class.getProtectionDomain().getCodeSource().getLocation().getPath();
		try {
			path = java.net.URLDecoder.decode(path, "UTF-8");
			File file = new File(path);
			path = file.getAbsolutePath();
			System.out.println("当前文件全称:" + path);
			System.out.println("当前文件名:" + file.getName());
			System.out.println("当前路径:" + file.getParentFile());
	...
	/**
	 * description:  开发模式使用(返回classes的绝对路径)
	 * @return  
	 * @author cuiyongxu 
	 * @update Nov 17, 2015
	 */
