...
import com.zbx.database.exception.DaoAccessException;
import com.zbx.database.model.MethodParam;
...
public class MybatisSession<O> {
	...
	public int update(String nameSpace, String query, Object... args) {
		...
		try {
			...
			throw new DaoAccessException("执行update方法出错", e);
		} finally {
			closeSqlSession(session);
		}
	}
	...
	public Page selectPage(String nameSpace, String query, MethodParam param) {
		...
		try {
			...
			throw new DaoAccessException("执行selectPage方法出错", e);
		} finally {
			closeSqlSession(session);
		}
		return pagingData;
